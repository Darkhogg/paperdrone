...
Bot.prototype.addPlugin = function addPlugin (plugin) {
    ...
    if (this._namedPlugins[name]) {
        this.logger.silly('ignoring already included plugin:', name);

    } else {
        this.logger.debug('adding plugin:', name);
...
Bot.prototype.processUpdate = function processUpdate (upd) {
    ...
    this.logger.silly('received update:', updid);
...
Bot.prototype.processTick = function processTick (when_) {
    ...
    this.logger.silly('received tick:', when.format());

    return this.emit('tick', when);
};

Bot.prototype._doProcessUpdate = function (update) {
    this.logger.verbose('processing update:', update.update_id);
...
Bot.prototype.pollUpdates = function pollUpdates (timeout_) {
    ...
    .then(function (fnd) {
        self.logger.silly('requesting updates starting at', fnd);
        ...
        if (updates.length > 0) {
            self.logger.silly('received %s updates', updates.length);
        }
...
Bot.prototype.setupTickLoop = function (interval_) {
    ...
    var interval = Math.ceil(interval_ || 1) * 60000;
