...
function KeyedStorage (owner, options) {
    ...
    var collName = (options.storage && options.storage.collection)
        ...
        : 'paperdrone_' + owner + '_keyed_storage';
...
KeyedStorage.prototype.get = function get (type, key) {
    ...
    self.logger.verbose('[KeyedStorage]  finding entry for %s:%s', type, key);

    return Promise.cast(self.mongo.findOne({'__type': type, '__key': key})).then(function (obj) {
        if (!obj) {
...
KeyedStorage.prototype.list = function list (type) {
    ...
    self.logger.verbose('[KeyedStorage]  listing entries for %s', type);

    return Promise.cast(self.mongo.find({'__type': type}).toArray().then(function (list) {
...
KeyedStorage.prototype.set = function set (type, key, value) {
    ...
    self.logger.verbose('[KeyedStorage]  setting entry for %s:%s', type, key, value);

    var obj = _.clone(value);
    obj.__type = type;
    obj.__key = key;

    return Promise.cast(self.mongo.findOne({'__type': type, '__key': key}).then(function (oldObj) {
        if (oldObj) {
            obj._id = oldObj._id;
        }
...
KeyedStorage.prototype.del = function del (type, key) {
    ...
    self.logger.verbose('[KeyedStorage]  removing entry for %s:%s', type, key);

    return Promise.cast(self.mongo.remove({'__type': type, '__key': key})).tap(function () {
...
KeyedStorage.prototype.empty = function empty (type) {
    ...
    self.logger.verbose('[KeyedStorage]  emptying entries for %s', type);

    return Promise.cast(self.mongo.remove({'__type': type})).tap(function () {
