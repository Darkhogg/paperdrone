...
module.exports = Plugin.define('Messages', function (bot, options) {
    ...
    bot.on('update', function ($evt, update) {
        ...
        bot.logger.debug('[Messages]  received "%s" from #%s (@%s, %s) on <#%s, %s>: ', msgType||actionType,
            message.from.id, username, message.from.full_name, message.chat.id, message.chat.full_name, message.text||'');
