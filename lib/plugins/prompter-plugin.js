...
class Prompter {
    ...
    async respond (chat, user, name, response) {
        ...
        if (prompt) {
            let $res = await this.bot.emit(['prompt.respond', `prompt.respond.${name}`], prompt, response);
            ...
            let result;
            for (let value of $res.values.filter(v => (v !== undefined))) {
    ...
    async onEnable (bot, options) {
        ...
        bot.on('message', async ($evt, msg, type) => {
            ...
            if (prompt && prompt.active) {
                ...
                $evt.stop();
                return await bot.prompter.respond(prompt.chat, prompt.user, prompt.name, msg);
            }
        }, EEE.PRIORITY.HIGH);
    }
}
