...
class Prompter {
    constructor (bot, options, logger) {
        this.bot = bot;
        this.logger = logger;
        this.collection = bot.mongo.collection('_prompter_');
    }
    ...
    async respond (chat, user, name, response) {
        ...
        if (prompt) {
            ...
            const actionCancelAll = false;
            const actionCancel = false;
            const actionRepeat = false;
            const actionContinue = false;
...
const PrompterPlugin = Plugin.define('prompter', ['messages', 'mongo'], {
    async start (config) {
        const prompter = new Prompter(this.bot, config, this.logger);
