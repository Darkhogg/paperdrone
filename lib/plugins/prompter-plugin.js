...
class Prompter {
  ...
  async respond (chat, user, name, response) {
    ...
    if (prompt) {
      ...
      if (actionCancelAll) {
        return await this.cancelAll(chat, user);
      }
      if (actionCancel) {
  ...
  async cancelAll (chat, user) {
    this.logger.trace('cancelling all prompts "%s:%s:*"', chat, user);

    const prompts = await this._removeAll(chat, user);

    for (const prompt of prompts.filter(p => p.active)) {
      await this.bot.emit(['prompt.cancel', `prompt.cancel.${prompt.name}`], prompt);
    }
    await this._checkAndActivateNext(chat, user);
  }
