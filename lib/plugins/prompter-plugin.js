...
const EE = require('../enhanced-emitter');
...
class Prompter {
    ...
    prompt (chat, user, name, data) {
        ...
        .then(() => this._checkAndActivateNext(chat, user, name));
    }
    ...
    complete (chat, user, name, result) {
        ...
        return this._remove(chat, user, name)
            .then((prompt) => Promise.all([
                this.emitter.emit(['prompt.complete', 'prompt.complete.'+name], prompt, result),
                this._checkAndActivateNext(chat, user),
            ]));
    }
    ...
    _checkAndActivateNext (chat, user) {
        ...
        return this.mongo.findAndModify(
            ...
            if (prompt && !prompt.active) {
                this.logger.silly('[Prompter]  activated prompt "%s:%s:%s"...', chat, user, prompt.name);

                return this.emitter.emit(['prompt.request', 'prompt.request.' + prompt.name], prompt, prompt.data);
            }
        });
    }
}
